...
for(const uid of ['unit-1','unit-2','unit-3','unit-4','unit-5','unit-7'])assert(home.includes(`data-unit="${uid}"`),`Released ${uid} switch missing`);
for(const uid of ['unit-1','unit-2','unit-3','unit-4','unit-5','unit-6','unit-7']){
  const u=course.units.find(x=>x.unit_id===uid); assert(u?.status==='STUDENT_READY',`${uid} not student-ready in course registry`);
}
for(const uid of ['unit-8'])assert(!home.includes(`data-unit="${uid}"`),`Unreleased ${uid} incorrectly switchable`);
...
console.log(JSON.stringify({
  ...
  refresh_resume:true,released_unit_switching:'Units 1-7',pixel_screenshot_validation:false
},null,2));
