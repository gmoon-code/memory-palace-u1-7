...
u7lock=AP/'unit-7'/'content-lock-f5.json'
...
for rel,digest in lock.get('runtime_file_sha256',{}).items():
    ...
    if current!=digest and later_runtime.get(rel)!=current and u7_runtime.get(rel)!=current:
        problems.append(f'F5 runtime file mismatch without later F6/Unit7 lock {rel}')
