...
for rel,digest in lock.get('runtime_file_sha256',{}).items():
    ...
    if current!=digest and later_runtime.get(rel)!=current and u7_runtime.get(rel)!=current: problems.append(f'F6 runtime hash mismatch without later Unit 6 lock: {rel}')
