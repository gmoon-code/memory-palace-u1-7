...
for rel,digest in f6_runtime.items():
    if sha(ROOT/rel)!=digest: problems.append(f'F6 runtime hash mismatch: {rel}')

health=client.get('/api/health').json()
if health.get('version')!='v2-apbio-0.28.0-u7-f6': problems.append(f"Unexpected API version: {health.get('version')}")
