...
for rel,digest in f6_runtime.items():
    ...
    if current!=digest and u7_runtime.get(rel)!=current:
        problems.append(f'F6 runtime hash mismatch without later Unit 7 lock: {rel}')

health=client.get('/api/health').json()
if health.get('version') not in {'v2-apbio-0.26.0-u6-f6','v2-apbio-0.27.0-u7-f5','v2-apbio-0.28.0-u7-f6'}: problems.append(f"Unexpected API version after Unit 6 F6: {health.get('version')}")
