...
def test_unit6_f6_content_and_runtime_lock():
    ...
    for rel,digest in lock['runtime_file_sha256'].items():
        ...
        assert current==digest or u7runtime.get(rel)==current

def test_unit6_f6_live_api():
    assert client.get('/api/health').json()['version'] in {'v2-apbio-0.26.0-u6-f6','v2-apbio-0.27.0-u7-f5','v2-apbio-0.28.0-u7-f6'}
