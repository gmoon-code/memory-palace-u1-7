...
def test_mainline_has_seven_student_ready_units():
    units=client.get('/api/course').json()['units']
    ready=[u['unit_id'] for u in units if u['status']=='STUDENT_READY']
    assert ready==['unit-1','unit-2','unit-3','unit-4','unit-5','unit-6','unit-7']

def test_mainline_runtime_version_is_u7_f6():
    assert client.get('/api/health').json()['version']=='v2-apbio-0.28.0-u7-f6'
